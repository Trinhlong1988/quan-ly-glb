...
export async function runFeeConfigSelfTest(): Promise<number> {
  ...
  // (2) 20 biểu phí: 4 đối tác × 5 loại thẻ (20)
  for (let pi = 0; pi < 4; pi++) {
    for (let ci = 0; ci < 5; ci++) {
      const r = await fee.setFeeRate({ partnerId: partners[pi].id, cardTypeId: cards[ci].id, phiMua: 1.02 + ci * 0.01, phiCaiMay: 1.0 + ci * 0.01, phiBan: 1.05 + ci * 0.01 });
      ok(`set phí P${pi}×${cards[ci].code} → ok`, r.ok === true, r);
    }
  }
  ok('list biểu phí = 20', (await fee.listFeeRates()).data?.length === 20);
  ...
  // (3) cột chênh lệch tính đúng (3)
  const setCL = await fee.setFeeRate({ partnerId: partners[0].id, cardTypeId: cards[0].id, phiMua: 1.5, phiCaiMay: 1.0, phiBan: 1.8 });
  ...
  ok('CL với KH = phiBán−phiCài = 0.8', rowCL != null && near(rowCL.clKh, 0.8), rowCL);

  // (4) round-trip 3 số thập phân 1.068 (1)
  await fee.setFeeRate({ partnerId: partners[1].id, cardTypeId: cards[0].id, phiMua: 1.068, phiCaiMay: 1.0, phiBan: 1.1 });
  ...
  await fee.setFeeRate({ partnerId: partners[0].id, cardTypeId: cards[0].id, phiMua: 2.0, phiCaiMay: 1.0, phiBan: 2.5 });
  ...
  const reAct = await fee.setFeeRate({ partnerId: partners[2].id, cardTypeId: cards[4].id, phiMua: 1.11, phiCaiMay: 1.0, phiBan: 1.2 });
  ...
  // (B) Biểu phí — validation/không liên kết/không tồn tại (14)
  const base = { partnerId: partners[0].id, cardTypeId: cards[0].id, phiMua: 1, phiCaiMay: 1, phiBan: 1 };
  ...
  ok('SAI phí bán âm → VALIDATION', (await fee.setFeeRate({ ...base, phiBan: -0.5 })).error === 'VALIDATION');
  ...
  ok('SAI phí bán >3 thập phân → VALIDATION', (await fee.setFeeRate({ ...base, phiBan: 1.23456 })).error === 'VALIDATION');
